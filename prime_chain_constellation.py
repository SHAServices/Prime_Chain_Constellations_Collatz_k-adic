...
def plot_constellation(chaindata, constellation_color='yellow', title="Prime Constellation"):
    ...
    # --- Data Processing ---
    n_arr = np.array(chaindata, dtype=float)
    ...
    ax.plot(theta, r, linestyle='-', color=constellation_color,
            linewidth=2.4, alpha=0.5, label='Constellation Lines')
    ...
    # Strictly fix the polar center at 0, and add a 5% margin outside the maximum radius
    ax.set_ylim(0, r.max() * 1.05)
